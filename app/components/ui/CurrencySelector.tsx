...
export function CurrencySelector({ 
  ...
  const defaultHook = useCurrency();

  const currentCurrency = selectedCurrency || defaultHook.selectedCurrency || {
    code: "INR",
    symbol: "₹",
    name: "Indian Rupee"
  };
  const handleCurrencyChange = onCurrencyChange || defaultHook.setSelectedCurrency;
...
export function useCurrency(): UseCurrencyReturn {
  ...
  useEffect(() => {
    const fetchExchangeRates = async () => {
      try {
        const stored = typeof window !== "undefined" ? localStorage.getItem("exchangeRates") : null;
        if (stored) {
          try {
            const { rates, timestamp } = JSON.parse(stored);
            // Cache valid for 12 hours (43200000 ms)
            if (rates && typeof rates === 'object' && timestamp && Date.now() - timestamp < 43200000) {
        ...
        if (response.ok && data.result === "success") {
          ...
          throw new Error("Exchange rate API quota or error: " + (data.result || response.status));
        }
      } catch (error) {
        // Fallback to cached or static rates without console spamming
        const stored = typeof window !== "undefined" ? localStorage.getItem("exchangeRates") : null;
        if (stored) {
          try {
  ...
  const convertPrice = (price: string): string => {
    ...
    // Use API rate if available, otherwise use fallback rate
    const rate = exchangeRates[selectedCurrency.code] || FALLBACK_RATES[selectedCurrency.code] || 1;
